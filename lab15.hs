...
-- part :: [[Int]] -> Bool
-- part bs = noDups bs && nontrivial bs && total bs bs && disjoint bs
--           where 
--             nontrivial (b:bs) | (bs == []) = (b /= [])
--                               | otherwise = if (b /= []) then nontrivial bs else False
--             total (b:bs) xs | (bs == []) = (xs == []) 
--                             | otherwise = if (element b xs) then total (bs (removeElement xs)) else (total bs xs)
--             disjoint bs = or [ [ ((x == y) || ((inters x y) = [])) | x <- bs] | y <- bs]
...
element b (x:xs) | (xs == []) = (b == x)
                 | otherwise = if (b == x) then True else x:(removeElement b xs)
